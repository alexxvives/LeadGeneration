...
/** Resolve bootstrap password: Wrangler/env secret, else one-time random. */
function resolveBootstrapAdminPassword(): string {
  const fromEnv = env.bootstrapAdminPassword();
  if (fromEnv) return fromEnv;
  const generated = crypto.randomUUID();
  console.warn(
    `[auth] BOOTSTRAP_ADMIN_PASSWORD unset — generated one-time admin password for ${BOOTSTRAP_ADMIN_EMAIL}: ${generated}`,
  );
  return generated;
}
 ...
 * `is_admin` user is already present. Password comes from
 * `BOOTSTRAP_ADMIN_PASSWORD` or a one-time generated UUID (logged once).
 * Rotate afterward via D1 `users.password_hash` or re-register.
 */
export async function ensureBootstrapAdmin(): Promise<void> {
  const d1 = await getD1Binding();
  if (d1) {
    if (await d1HasAdmin(d1)) return;
    const passwordHash = await hashPassword(resolveBootstrapAdminPassword());
